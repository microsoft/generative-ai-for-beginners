...
LLMs صرف متن تخلیق کرنے تک محدود نہیں ہیں۔ متن کی وضاحتوں سے تصاویر تخلیق کرنا بھی ممکن ہے۔ تصاویر کو ایک موڈیلٹی کے طور پر استعمال کرنا مختلف شعبوں جیسے میڈٹیک، آرکیٹیکچر، سیاحت، گیم ڈیولپمنٹ اور دیگر میں بہت مفید ہو سکتا ہے۔ اس باب میں، ہم دو مشہور تصویری تخلیق کے ماڈلز، DALL-E اور Midjourney، پر غور کریں گے۔
...
تصویری تخلیق کی ایپلیکیشنز جنریٹو AI کی صلاحیتوں کو دریافت کرنے کا ایک بہترین طریقہ ہیں۔ ان کا استعمال مختلف مقاصد کے لیے کیا جا سکتا ہے، جیسے:
...
اس سبق کے حصے کے طور پر، ہم اپنے اسٹارٹ اپ، Edu4All، کے ساتھ کام جاری رکھیں گے۔ طلباء اپنی اسائنمنٹس کے لیے تصاویر تخلیق کریں گے، تصاویر کی نوعیت طلباء پر منحصر ہے، لیکن وہ اپنی پریوں کی کہانی کے لیے تصاویر بنا سکتے ہیں، اپنے کہانی کے لیے نیا کردار تخلیق کر سکتے ہیں یا اپنے خیالات اور تصورات کو بصری شکل دے سکتے ہیں۔
...
> "صبح کی روشنی میں ایفل ٹاور کے پاس ایک کتا"

## DALL-E اور Midjourney کیا ہیں؟

[DALL-E](https://openai.com/dall-e-2?WT.mc_id=academic-105485-koreyst) اور [Midjourney](https://www.midjourney.com/?WT.mc_id=academic-105485-koreyst) دو مشہور تصویری تخلیق کے ماڈلز ہیں، جو آپ کو پرامپٹس کے ذریعے تصاویر تخلیق کرنے کی اجازت دیتے ہیں۔
...
- **Diffused attention**، ایک ماڈل ہے جو ایمبیڈنگز سے تصاویر تخلیق کرتا ہے۔ DALL-E کو تصاویر اور متن کے ڈیٹا سیٹ پر تربیت دی گئی ہے اور اسے متن کی وضاحتوں سے تصاویر تخلیق کرنے کے لیے استعمال کیا جا سکتا ہے۔ مثال کے طور پر، DALL-E کو ایک ٹوپی میں بلی یا ایک موہاک والے کتے کی تصویر تخلیق کرنے کے لیے استعمال کیا جا سکتا ہے۔

### Midjourney

Midjourney بھی DALL-E کی طرح کام کرتا ہے، یہ متن کے پرامپٹس سے تصاویر تخلیق کرتا ہے۔ Midjourney کو بھی ایسے پرامپٹس کے ذریعے تصاویر تخلیق کرنے کے لیے استعمال کیا جا سکتا ہے جیسے "ایک ٹوپی میں بلی" یا "موہاک والا کتا"۔
...
_تصویر کا کریڈٹ: ویکیپیڈیا، تصویر Midjourney کے ذریعے تخلیق کی گئی_
...
اس عمل کے ذریعے، DALL-E تخلیق کردہ تصویر میں خصوصیات، اشیاء، اور دیگر عناصر کو کنٹرول کرتا ہے۔ تاہم، DALL-E 2 اور 3 تخلیق کردہ تصویر پر زیادہ کنٹرول فراہم کرتے ہیں۔
...
اگر پہلے سے نہیں کیا گیا، تو [Microsoft Learn](https://learn.microsoft.com/azure/ai-foundry/openai/how-to/create-resource?pivots=web-portal) صفحے پر دی گئی ہدایات پر عمل کریں تاکہ Azure OpenAI ریسورس اور ماڈل تخلیق کریں۔ DALL-E 3 کو ماڈل کے طور پر منتخب کریں۔

## ایپلیکیشن تخلیق کریں
...
- آخر میں، ہم تصویر کھولتے ہیں اور اسے معیاری تصویر دیکھنے والے کے ذریعے دکھاتے ہیں:
...
آئیے اس کوڈ پر غور کریں جو تصویر تخلیق کرتا ہے:
...
- **prompt**، وہ متن پرامپٹ ہے جو تصویر تخلیق کرنے کے لیے استعمال ہوتا ہے۔ اس مثال میں، ہم پرامپٹ "گھوڑے پر خرگوش، ہاتھ میں لالی پاپ، دھندلے میدان میں جہاں نرگس کے پھول اگتے ہیں" استعمال کر رہے ہیں۔
- **size**، تخلیق کردہ تصویر کا سائز ہے۔ اس مثال میں، ہم 1024x1024 پکسلز کی تصویر تخلیق کر رہے ہیں۔
- **n**، تخلیق کردہ تصاویر کی تعداد ہے۔ اس مثال میں، ہم دو تصاویر تخلیق کر رہے ہیں۔
- **temperature**، ایک پیرامیٹر ہے جو جنریٹو AI ماڈل کے آؤٹ پٹ کی بے ترتیبی کو کنٹرول کرتا ہے۔ temperature کی قدر 0 اور 1 کے درمیان ہوتی ہے، جہاں 0 کا مطلب ہے کہ آؤٹ پٹ متعین ہے اور 1 کا مطلب ہے کہ آؤٹ پٹ بے ترتیب ہے۔ ڈیفالٹ قدر 0.7 ہے۔

تصاویر کے ساتھ مزید کام کرنے کے لیے اگلے حصے میں مزید تفصیلات دی جائیں گی۔

## تصویری تخلیق کی اضافی صلاحیتیں

آپ نے دیکھا کہ ہم نے چند لائنز Python میں استعمال کرتے ہوئے تصویر تخلیق کی۔ تاہم، تصاویر کے ساتھ مزید کام بھی کیے جا سکتے ہیں۔

آپ درج ذیل کام بھی کر سکتے ہیں:

- **ترمیم کریں**۔ موجودہ تصویر، ماسک اور پرامپٹ فراہم کر کے آپ تصویر میں تبدیلی کر سکتے ہیں۔ مثال کے طور پر، آپ تصویر کے کسی حصے میں کچھ شامل کر سکتے ہیں۔ فرض کریں کہ ہماری خرگوش کی تصویر ہے، آپ خرگوش کو ایک ٹوپی پہنا سکتے ہیں۔ یہ کام تصویر، ماسک (تبدیلی کے علاقے کی شناخت کے لیے) اور متن پرامپٹ فراہم کر کے کیا جاتا ہے۔
> نوٹ: یہ DALL-E 3 میں سپورٹ نہیں ہے۔

یہاں GPT Image کا ایک مثال ہے:
...
جیسا کہ آپ دیکھ سکتے ہیں، تصاویر ملتی جلتی ہیں، لیکن ایک جیسی نہیں ہیں۔ آئیے Temperature کی قدر کو 0.1 پر تبدیل کریں اور دیکھیں کیا ہوتا ہے:
...
تو آئیے آؤٹ پٹ کو زیادہ متعین بنانے کی کوشش کرتے ہیں۔ ہم نے جو دو تصاویر تخلیق کیں ان سے مشاہدہ کیا کہ پہلی تصویر میں خرگوش ہے اور دوسری تصویر میں گھوڑا ہے، لہذا تصاویر میں بہت فرق ہے۔

آئیے اپنے کوڈ کو تبدیل کریں اور Temperature کو 0 پر سیٹ کریں، کچھ اس طرح:
...
## اپنی ایپلیکیشن کے لیے حدود مقرر کرنے کے لیے میٹا پرامپٹس کا استعمال
...
مثال کے طور پر، ہم ایسی تصاویر تخلیق نہیں کرنا چاہتے جو کام کے لیے محفوظ نہ ہوں یا بچوں کے لیے مناسب نہ ہوں۔
...
ہم نے اس سبق کے آغاز میں Edu4All کا تعارف کرایا۔ اب وقت ہے کہ طلباء کو ان کی اسائنمنٹس کے لیے تصاویر تخلیق کرنے کے قابل بنایا جائے۔

طلباء اپنی اسائنمنٹس کے لیے یادگاروں پر مشتمل تصاویر تخلیق کریں گے، یادگاروں کی نوعیت طلباء پر منحصر ہے۔ طلباء کو اس کام میں اپنی تخلیقی صلاحیتوں کا استعمال کرنے کے لیے کہا گیا ہے تاکہ ان یادگاروں کو مختلف سیاق و سباق میں رکھا جا سکے۔
