...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T21:46:28+00:00",
...
LLM'lerin yalnızca metin oluşturma yeteneklerinden daha fazlası var. Metin açıklamalarından görüntü oluşturmak da mümkündür. Görüntülerin bir modalite olarak kullanılması, MedTech, mimarlık, turizm, oyun geliştirme ve daha birçok alanda oldukça faydalı olabilir. Bu bölümde, en popüler iki görüntü oluşturma modeli olan DALL-E ve Midjourney'i inceleyeceğiz.
...
- Görüntü oluşturma uygulaması nasıl geliştirilir.

## Öğrenme Hedefleri

Bu dersi tamamladıktan sonra şunları yapabileceksiniz:

- Görüntü oluşturma uygulaması geliştirmek.
...
[DALL-E](https://openai.com/dall-e-2?WT.mc_id=academic-105485-koreyst) ve [Midjourney](https://www.midjourney.com/?WT.mc_id=academic-105485-koreyst), en popüler görüntü oluşturma modellerinden ikisidir ve istemler kullanarak görüntüler oluşturmanıza olanak tanır.

### DALL-E

Öncelikle DALL-E ile başlayalım; bu, metin açıklamalarından görüntüler oluşturan bir Üretken Yapay Zeka modelidir.
...
- **CLIP**, görüntüler ve metinlerden sayısal veri temsilleri olan gömme (embedding) oluşturan bir modeldir.

- **Diffused attention**, gömmelerden görüntüler oluşturan bir modeldir. DALL-E, görüntüler ve metinlerden oluşan bir veri seti üzerinde eğitilmiştir ve metin açıklamalarından görüntüler oluşturmak için kullanılabilir. Örneğin, DALL-E bir şapka takmış kedi veya mohawk saçlı bir köpek görüntüsü oluşturmak için kullanılabilir.
...
Öncelikle [DALL-E](https://arxiv.org/pdf/2102.12092.pdf?WT.mc_id=academic-105485-koreyst). DALL-E, _oto-regresif transformer_ mimarisiyle çalışan bir Üretken Yapay Zeka modelidir.

Bir _oto-regresif transformer_, bir modelin metin açıklamalarından görüntü oluşturma şeklini tanımlar; bir pikseli bir seferde oluşturur ve ardından oluşturulan pikselleri bir sonraki pikseli oluşturmak için kullanır. Bu işlem, görüntü tamamlanana kadar bir sinir ağı içinde birden fazla katmandan geçer.
...
- **python-dotenv**, gizli bilgilerinizi _.env_ dosyasında koddan uzak tutmak için bu kütüphaneyi kullanmanız şiddetle tavsiye edilir.
...
Henüz yapmadıysanız, bir Azure OpenAI kaynağı ve modeli oluşturmak için [Microsoft Learn](https://learn.microsoft.com/azure/ai-foundry/openai/how-to/create-resource?pivots=web-portal) sayfasındaki talimatları izleyin. Model olarak DALL-E 3'ü seçin.
...
1. _.env_ adlı bir dosya oluşturun ve şu içeriği ekleyin:
   ...
   Bu bilgiyi Azure OpenAI Foundry Portal'daki kaynağınızın "Deployments" bölümünde bulun.
...
1. Ardından sanal bir ortam oluşturun ve kütüphaneleri yükleyin:
   ...
   Windows için sanal ortam oluşturmak ve etkinleştirmek için şu komutları kullanın:
...
- Son olarak, görüntüyü açıyoruz ve standart görüntü görüntüleyiciyi kullanarak görüntülüyoruz:
...
Görüntüyü oluşturan koda daha yakından bakalım:
...
- **prompt**, görüntüyü oluşturmak için kullanılan metin istemidir. Bu durumda, "Sisli bir çayırda nergislerin büyüdüğü yerde, at üzerinde lolipop tutan bir tavşan" istemini kullanıyoruz.
- **size**, oluşturulan görüntünün boyutudur. Bu durumda, 1024x1024 piksel boyutunda bir görüntü oluşturuyoruz.
- **n**, oluşturulan görüntülerin sayısıdır. Bu durumda, iki görüntü oluşturuyoruz.
- **temperature**, Üretken Yapay Zeka modelinin çıktısının rastgeleliğini kontrol eden bir parametredir. Sıcaklık, 0 ile 1 arasında bir değerdir; 0, çıktının deterministik olduğu anlamına gelirken, 1, çıktının rastgele olduğu anlamına gelir. Varsayılan değer 0.7'dir.
...
- **Varyasyonlar oluşturun**. Mevcut bir görüntüyü alıp farklı bağlamlarda varyasyonlar oluşturmasını isteyebilirsiniz. Bir varyasyon oluşturmak için bir görüntü ve bir metin istemi sağlayarak şu şekilde kod yazabilirsiniz:
...
Sıcaklık, Üretken Yapay Zeka modelinin çıktısının rastgeleliğini kontrol eden bir parametredir. Sıcaklık, 0 ile 1 arasında bir değerdir; 0, çıktının deterministik olduğu anlamına gelirken, 1, çıktının rastgele olduğu anlamına gelir. Varsayılan değer 0.7'dir.

Sıcaklığın nasıl çalıştığını bir örnekle inceleyelim; bu istemi iki kez çalıştırarak:

> İstem: "Sisli bir çayırda nergislerin büyüdüğü yerde, at üzerinde lolipop tutan bir tavşan"
...
![At üzerinde lolipop tutan tavşan](../../../translated_images/v2-generated-image.33f55a3714efe61dc19622c869ba6cd7d6e6de562e26e95b5810486187aace39.tr.png)
...
Demo'muzla müşterilerimiz için zaten görüntüler oluşturabiliyoruz. Ancak, uygulamamız için bazı sınırlar oluşturmalıyız.

Örneğin, iş yerinde uygun olmayan veya çocuklar için uygun olmayan görüntüler oluşturmak istemiyoruz.

Bunu _meta istemler_ ile yapabiliriz. Meta istemler, Üretken Yapay Zeka modelinin çıktısını kontrol etmek için kullanılan metin istemleridir. Örneğin, meta istemleri kullanarak çıktıyı kontrol edebilir ve oluşturulan görüntülerin iş yerinde uygun olmasını veya çocuklar için uygun olmasını sağlayabiliriz.

### Nasıl çalışır?

Peki, meta istemler nasıl çalışır?

Meta istemler, Üretken Yapay Zeka modelinin çıktısını kontrol etmek için kullanılan metin istemleridir; metin isteminden önce konumlandırılırlar ve modelin çıktısını kontrol etmek için kullanılırlar. Uygulamalara gömülerek modelin çıktısını kontrol etmek için kullanılırlar. İstem girdisi ve meta istem girdisini tek bir metin isteminde kapsar.
...
Öğrenciler, değerlendirmeleri için anıtlar içeren görüntüler oluşturacaklar; hangi anıtların oluşturulacağı tamamen öğrencilere bağlıdır. Öğrencilerden bu görevde yaratıcılıklarını kullanarak bu anıtları farklı bağlamlara yerleştirmeleri istenir.
