...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-25T16:54:59+00:00",
...
Transkripsiyon veri hazırlık betikleri, YouTube video transkriptlerini indirir ve bunları OpenAI Embeddings ve Fonksiyonlar ile Semantik Arama örneğinde kullanılmak üzere hazırlar.
...
## Gerekli Azure OpenAI Servis kaynaklarını oluşturun

> [!IMPORTANT]
> OpenAI ile uyumluluğu sağlamak için Azure CLI'yi en son sürüme güncellemenizi öneririz.
> [Dokümantasyon](https://learn.microsoft.com/cli/azure/update-azure-cli?WT.mc_id=academic-105485-koreyst) adresine bakın.
...
1. Bir Azure OpenAI Servis kaynağı oluşturun.
...
`Windows Start` > `Sistem ortam değişkenlerini düzenle` > `Ortam Değişkenleri` > `Kullanıcı değişkenleri` for [USER] > `Yeni` eklemenizi öneririz.
...
Aşağıdaki dışa aktarmaları `~/.bashrc` or `~/.zshrc` dosyanıza eklemenizi öneririz.
...
1. [Git istemcisini](https://git-scm.com/downloads?WT.mc_id=academic-105485-koreyst) yükleyin, eğer zaten yüklü değilse.
1. `Terminal` penceresinden, örneği tercih ettiğiniz repo klasörüne klonlayın.
