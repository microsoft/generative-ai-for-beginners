...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T21:15:28+00:00",
...
- نحوه ساخت یک برنامه تولید تصویر.
...
- **ویرایش و ترکیب تصاویر**. شما می‌توانید تصاویر را برای موارد مختلفی مانند ویرایش و ترکیب تصاویر تولید کنید.

- **کاربرد در صنایع مختلف**. همچنین می‌توان از این برنامه‌ها برای تولید تصاویر در صنایع مختلف مانند فناوری پزشکی، گردشگری، توسعه بازی و موارد دیگر استفاده کرد.
...
- **توجه پراکنده**، مدلی است که تصاویر را از تعبیه‌ها تولید می‌کند. DALL-E بر روی مجموعه داده‌ای از تصاویر و متن آموزش داده شده است و می‌توان از آن برای تولید تصاویر از توضیحات متنی استفاده کرد. برای مثال، DALL-E می‌تواند تصاویر یک گربه با کلاه یا یک سگ با موهاک تولید کند.

### Midjourney

Midjourney به شیوه‌ای مشابه DALL-E کار می‌کند و تصاویر را از پرومت‌های متنی تولید می‌کند. Midjourney نیز می‌تواند با استفاده از پرومت‌هایی مانند "گربه با کلاه" یا "سگ با موهاک" تصاویر تولید کند.
...
یک _ترانسفورمر خودبازگشتی_ نحوه تولید تصاویر از توضیحات متنی را تعریف می‌کند. این مدل یک پیکسل را در یک زمان تولید می‌کند و سپس از پیکسل‌های تولید شده برای تولید پیکسل بعدی استفاده می‌کند. این فرآیند از طریق لایه‌های مختلف در یک شبکه عصبی ادامه می‌یابد تا تصویر کامل شود.
...
اگر هنوز انجام نشده است، دستورالعمل‌های صفحه [Microsoft Learn](https://learn.microsoft.com/azure/ai-foundry/openai/how-to/create-resource?pivots=web-portal) را دنبال کنید تا یک منبع و مدل Azure OpenAI ایجاد کنید. مدل DALL-E 3 را انتخاب کنید.
...
- در نهایت، تصویر را باز کرده و با استفاده از نمایشگر استاندارد تصویر آن را نمایش می‌دهیم:
...
- **prompt**، پرومت متنی است که برای تولید تصویر استفاده می‌شود. در این مورد، از پرومت "خرگوش روی اسب، در حال نگه داشتن آب‌نبات چوبی، در یک چمنزار مه‌آلود که نرگس‌ها در آن رشد می‌کنند" استفاده می‌کنیم.
...
کارهای بیشتری می‌توان با تصاویر انجام داد که در بخش بعدی به آن‌ها خواهیم پرداخت.

## قابلیت‌های اضافی تولید تصویر

تا اینجا دیدید که چگونه توانستیم با چند خط کد در پایتون یک تصویر تولید کنیم. با این حال، کارهای بیشتری می‌توان با تصاویر انجام داد.

شما همچنین می‌توانید موارد زیر را انجام دهید:

- **ویرایش تصاویر**. با ارائه یک تصویر موجود، یک ماسک و یک پرومت، می‌توانید تصویر را تغییر دهید. برای مثال، می‌توانید چیزی را به بخشی از تصویر اضافه کنید. تصور کنید تصویر خرگوش ما، می‌توانید یک کلاه به خرگوش اضافه کنید. نحوه انجام این کار با ارائه تصویر، یک ماسک (شناسایی بخش مورد نظر برای تغییر) و یک پرومت متنی برای توضیح تغییر است.
...
- **ایجاد تغییرات**. ایده این است که یک تصویر موجود را بگیرید و درخواست کنید که تغییراتی ایجاد شود. برای ایجاد تغییرات، یک تصویر و یک پرومت متنی ارائه می‌دهید و کدی مانند زیر استفاده می‌کنید:
...
> پرومت: "خرگوش روی اسب، در حال نگه داشتن آب‌نبات چوبی، در یک چمنزار مه‌آلود که نرگس‌ها در آن رشد می‌کنند"
...
بنابراین کد خود را تغییر دهیم و دما را به 0 تنظیم کنیم، به این صورت:
...
می‌توانیم این کار را با _متاپرومت‌ها_ انجام دهیم. متاپرومت‌ها پرومت‌های متنی هستند که برای کنترل خروجی مدل هوش مصنوعی مولد استفاده می‌شوند. برای مثال، می‌توانیم از متاپرومت‌ها برای کنترل خروجی استفاده کنیم و اطمینان حاصل کنیم که تصاویر تولید شده مناسب محیط کار یا کودکان هستند.

### چگونه کار می‌کند؟

حالا، متاپرومت‌ها چگونه کار می‌کنند؟

متاپرومت‌ها پرومت‌های متنی هستند که برای کنترل خروجی مدل هوش مصنوعی مولد استفاده می‌شوند. این پرومت‌ها قبل از پرومت متنی قرار می‌گیرند و برای کنترل خروجی مدل استفاده می‌شوند و در برنامه‌ها برای کنترل خروجی مدل جاسازی می‌شوند. پرومت ورودی و متاپرومت ورودی را در یک پرومت متنی واحد قرار می‌دهند.
...
دانش‌آموزان تصاویر را برای ارزیابی‌های خود که شامل بناهای تاریخی است ایجاد خواهند کرد. اینکه چه بناهایی ایجاد کنند به خود دانش‌آموزان بستگی دارد. از دانش‌آموزان خواسته شده است که در این وظیفه خلاقیت خود را به کار گیرند و این بناها را در زمینه‌های مختلف قرار دهند.
