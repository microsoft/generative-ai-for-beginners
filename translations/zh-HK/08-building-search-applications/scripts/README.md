...
轉錄數據準備腳本會下載 YouTube 影片轉錄文本，並將其準備好以用於帶有 OpenAI 嵌入和函數的語義搜索範例。

轉錄數據準備腳本已在最新發行的 Windows 11、macOS Ventura 和 Ubuntu 22.04（及以上版本）上測試。
...
> [!NOTE]
> 這些指示中，我們使用的是位於 East US 的名為 "semantic-video-search" 的資源群組。
> 您可以更改資源群組的名稱，但若更改資源的地點，
...
1. 部署以下模型：
   - `text-embedding-ada-002` 版本 `2` 或以上，名稱為 `text-embedding-ada-002`
   - `gpt-5-mini`，名稱為 `gpt-5-mini`
...
運行 YouTube 轉錄數據準備腳本需要以下環境變數。

### 在 Windows 上

建議將變數新增到您的 `使用者` 環境變數中。
`Windows 開始` > `編輯系統環境變數` > `環境變數` > [USER] 的 `使用者變數` > `新增`。
...
建議將以下 export 新增到您的 `~/.bashrc` 或 `~/.zshrc` 檔案中。
...
1. 從 `終端機` 視窗將範例複製 (clone) 至您偏好的資料夾。
