...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T21:41:37+00:00",
...
Como parte desta lição, continuaremos trabalhando com nossa startup, Edu4All. Os alunos criarão imagens para suas avaliações. Quais imagens serão criadas fica a critério dos alunos, mas elas podem ser ilustrações para seu próprio conto de fadas, um novo personagem para sua história ou algo que os ajude a visualizar suas ideias e conceitos.
...
Um _transformer autoregressivo_ define como um modelo gera imagens a partir de descrições textuais, gerando um pixel de cada vez e usando os pixels gerados para gerar o próximo pixel. Passando por várias camadas em uma rede neural, até que a imagem esteja completa.
...
Então, vamos tentar tornar a resposta mais determinística. Podemos observar nas duas imagens geradas que na primeira imagem há um coelho e na segunda imagem há um cavalo, então as imagens variam bastante.
...
Podemos fazer isso com _metaprompts_. Metaprompts são prompts textuais usados para controlar a saída de um modelo de IA Generativa. Por exemplo, podemos usar metaprompts para controlar a saída e garantir que as imagens geradas sejam apropriadas para o trabalho ou adequadas para crianças.

### Como funciona?

Agora, como funcionam os metaprompts?

Metaprompts são prompts textuais usados para controlar a saída de um modelo de IA Generativa. Eles são posicionados antes do prompt textual e são usados para controlar a saída do modelo, sendo incorporados em aplicativos para controlar a saída do modelo. Encapsulando o input do prompt e o input do metaprompt em um único prompt textual.
...
Os alunos criarão imagens para suas avaliações contendo monumentos. Quais monumentos serão criados fica a critério dos alunos. Eles são incentivados a usar sua criatividade nesta tarefa para colocar esses monumentos em diferentes contextos.
