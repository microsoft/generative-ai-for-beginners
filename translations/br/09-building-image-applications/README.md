...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-25T17:18:09+00:00",
...
Há mais nos LLMs do que geração de texto. Também é possível gerar imagens a partir de descrições textuais. Ter imagens como uma modalidade pode ser altamente útil em várias áreas, como MedTech, arquitetura, turismo, desenvolvimento de jogos e mais. Neste capítulo, vamos explorar os dois modelos de geração de imagens mais populares, DALL-E e Midjourney.
...
- Definir limites para sua aplicação com metapromptes.
...
- **Aplicado a uma variedade de indústrias**. Elas também podem ser usadas para gerar imagens para diversas indústrias, como MedTech, Turismo, Desenvolvimento de Jogos e mais.
...
Aqui está o que os estudantes da Edu4All poderiam gerar, por exemplo, se estiverem trabalhando em sala de aula sobre monumentos:
...
Midjourney funciona de forma semelhante ao DALL-E, gerando imagens a partir de prompts textuais. Midjourney também pode ser usado para gerar imagens usando prompts como "um gato com um chapéu", ou um "cachorro com um moicano".
...
Um _transformer autoregressivo_ define como um modelo gera imagens a partir de descrições textuais, ele gera um pixel de cada vez, e depois usa os pixels gerados para gerar o próximo pixel. Passando por várias camadas em uma rede neural, até que a imagem esteja completa.
...
- **python-dotenv**, é altamente recomendável usar esta biblioteca para manter seus segredos em um arquivo _.env_ separado do código.
...
1. Crie um arquivo _.env_ com o seguinte conteúdo:
   ...
   Localize esta informação no Portal Azure para seu recurso na seção "Chaves e Endpoint".
...
- Depois disso, definimos o endpoint, chave para a API OpenAI, versão e tipo.
...
- Por último, abrimos a imagem e usamos o visualizador de imagens padrão para exibí-la:

### Mais detalhes sobre a geração da imagem

Vamos olhar o código que gera a imagem em mais detalhes:

- **prompt**, é o prompt de texto que é usado para gerar a imagem. Neste caso, estamos usando o prompt "Coelho no cavalo, segurando um pirulito, em um prado enevoado onde crescem narcisos".
...
Você viu até agora como fomos capazes de gerar uma imagem usando algumas linhas em Python. No entanto, há mais coisas que você pode fazer com imagens.
...
- **Criar variações**. A ideia é que você pegue uma imagem existente e peça que variações sejam criadas. Para criar uma variação, você fornece uma imagem e um prompt de texto e um código assim:
...
Vamos olhar um exemplo de como a temperatura funciona, executando este prompt duas vezes:

> Prompt: "Coelho no cavalo, segurando um pirulito, em um prado enevoado onde crescem narcisos"

Agora vamos executar o mesmo prompt apenas para ver que não obteremos a mesma imagem duas vezes:

Como você pode ver, as imagens são semelhantes, mas não são iguais. Vamos tentar mudar o valor da temperatura para 0.1 e ver o que acontece:

### Mudando a temperatura

Então, vamos tentar tornar a resposta mais determinística. Pudemos observar das duas imagens que geramos que na primeira imagem, há um coelho e na segunda imagem, há um cavalo, então as imagens variam muito.
...
## Como definir limites para sua aplicação com metapromptes
...
Podemos fazer isso com _metapromptes_. Metapromptes são prompts de texto que são usados para controlar o resultado de um modelo de IA Generativa. Por exemplo, podemos usar metapromptes para controlar o resultado e garantir que as imagens geradas sejam seguras para o trabalho ou apropriadas para crianças.

### Como funciona?

Agora, como os metapromptes funcionam?

Metapromptes são prompts de texto que são usados para controlar o resultado de um modelo de IA Generativa, eles são posicionados antes do prompt de texto e são usados para controlar o resultado do modelo e incorporados em aplicações para controlar o resultado do modelo. Encapsulando a entrada do prompt e a entrada do metaprompt em um único prompt de texto.

Um exemplo de um metaprompt seria o seguinte:

Agora, vamos ver como podemos usar metapromptes em nossa demonstração.
