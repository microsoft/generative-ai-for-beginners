...
컴퓨터나 코드스페이스에 직접 모든 걸 설치하는 대신 [컨테이너](https://en.wikipedia.org/wiki/Containerization_%28computing%29?WT.mc_id=academic-105485-koreyst)를 사용하는 방법도 있습니다. 코스 저장소 내 특별한 `.devcontainer` 폴더는 VS Code가 프로젝트를 컨테이너 내에서 설정하도록 돕습니다. Codespaces가 아닌 경우 Docker 설치가 필요하며, 다소 작업이 복잡할 수 있어 컨테이너 경험이 있는 분께만 권장합니다.
