...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-26T03:24:59+00:00",
...
- Comprendre les cas d'utilisation et scénarios pour chaque modèle
- Exemple de code pour montrer les caractéristiques uniques de chaque modèle

## La famille de modèles Meta

Dans cette leçon, nous allons explorer 2 modèles de la famille Meta ou "Troupeau de Llama" - Llama 3.1 et Llama 3.2

Ces modèles existent en différentes variantes et sont disponibles sur la place de marché des modèles GitHub. Voici plus de détails sur l'utilisation des modèles GitHub pour [prototyper avec des modèles AI](https://docs.github.com/en/github-models/prototyping-with-ai-models?WT.mc_id=academic-105485-koreyst).
...
Le modèle est une amélioration par rapport à la version précédente Llama 3 en offrant :
...
- Appel de fonction native - la capacité d'appeler des outils et fonctions externes en dehors du flux de travail LLM
- Meilleure performance RAG - grâce à la fenêtre de contexte plus grande
...
Llama 3.1 a été affiné pour être plus efficace dans l'appel de fonctions ou d'outils. Il dispose également de deux outils intégrés que le modèle peut identifier comme devant être utilisés en fonction de l'invite de l'utilisateur. Ces outils sont :
...
- Le LLM répondra avec un appel d'outil à l'outil Brave Search qui ressemblera à ceci `<|python_tag|>brave_search.call(query="Stockholm weather")`

*Remarque : Cet exemple ne fait que l'appel d'outil, si vous souhaitez obtenir les résultats, vous devrez créer un compte gratuit sur la page API de Brave et définir la fonction elle-même*
...
Bien qu'étant un LLM, une limitation que Llama 3.1 a est la multimodalité. C'est-à-dire, être capable d'utiliser différents types d'entrée tels que des images comme invites et de fournir des réponses. Cette capacité est l'une des principales caractéristiques de Llama 3.2. Ces caractéristiques incluent également :

- Multimodalité - a la capacité d'évaluer à la fois des invites texte et image
- Variations de petite à moyenne taille (11B et 90B) - cela offre des options de déploiement flexibles,
- Variations texte uniquement (1B et 3B) - cela permet au modèle d'être déployé sur des appareils en périphérie / mobiles et offre une faible latence

Le support multimodal représente un grand pas en avant dans le monde des modèles open source. L'exemple de code ci-dessous prend à la fois une image et une invite texte pour obtenir une analyse de l'image par Llama 3.2 90B.
