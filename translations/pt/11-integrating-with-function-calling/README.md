...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-25T19:51:25+00:00",
...
- Configurar chamadas de função usando o Azure OpenAI Service.
- Projetar chamadas de função eficazes para o caso de uso da sua aplicação.

## Cenário: Melhorando nosso chatbot com funções

Para esta lição, queremos construir um recurso para nossa startup de educação que permita aos usuários usar um chatbot para encontrar cursos técnicos. Recomendaremos cursos que se ajustem ao seu nível de habilidade, função atual e tecnologia de interesse.
...
Antes das chamadas de função, as respostas de um LLM eram não estruturadas e inconsistentes. Os desenvolvedores eram obrigados a escrever códigos de validação complexos para garantir que pudessem lidar com cada variação de uma resposta. Os usuários não conseguiam obter respostas como "Qual é o clima atual em Estocolmo?". Isso porque os modelos eram limitados ao tempo em que os dados foram treinados.

Chamadas de Função é um recurso do Azure OpenAI Service para superar as seguintes limitações:
...
> Recomendamos que você use o [notebook incluído](../../../11-integrating-with-function-calling/python/aoai-assignment.ipynb) se quiser executar o cenário abaixo. Você também pode apenas ler enquanto tentamos ilustrar um problema onde as funções podem ajudar a resolver o problema.
...
1. Criar uma conexão com nosso recurso Azure OpenAI:
   ...
   Abaixo está um código Python para configurar nossa conexão com o Azure OpenAI onde definimos `api_type`, `api_base`, `api_version` and `api_key`.
...
1. Creating two student descriptions using variables `student_1_description` and `student_2_description`.
   ...
   Queremos enviar as descrições dos estudantes acima para um LLM para analisar os dados. Esses dados podem ser usados mais tarde em nossa aplicação e serem enviados para uma API ou armazenados em um banco de dados.
...
1. Lastly, we can convert the response to JSON format by calling `json.loads`:
   ...
   Embora os prompts sejam os mesmos e as descrições sejam semelhantes, vemos valores dos `Grades` property formatted differently, as we can sometimes get the format `3.7` or `3.7 GPA` for example.

   This result is because the LLM takes unstructured data in the form of the written prompt and returns also unstructured data. We need to have a structured format so that we know what to expect when storing or using this data

So how do we solve the formatting problem then? By using functional calling, we can make sure that we receive structured data back. When using function calling, the LLM does not actually call or run any functions. Instead, we create a structure for the LLM to follow for its responses. We then use those structured responses to know what function to run in our applications.

![function flow](../../../translated_images/Function-Flow.083875364af4f4bb69bd6f6ed94096a836453183a71cf22388f50310ad6404de.pt.png)
...
![LLM Flow](../../../translated_images/LLM-Flow.3285ed8caf4796d7343c02927f52c9d32df59e790f6e440568e2e951f6ffa5fd.pt.png)
...
Ao atribuir diferentes papéis, fica claro para o LLM se é o sistema dizendo algo ou o usuário, o que ajuda a construir um histórico de conversa que o LLM pode usar como base.
...
Here's some code below where we call `ChatCompletion.create`, note how we set `functions=functions` and `function_call="auto"` e assim dar ao LLM a escolha de quando chamar as funções que fornecemos:
...
A resposta que chega agora se parece com isto:
...
1. Agora definiremos a função que chamará a API Microsoft Learn para obter uma lista de cursos:
   ...
   Note como agora criamos uma função Python real que mapeia para os nomes de função introduzidos nas `functions` variable. We're also making real external API calls to fetch the data we need. In this case, we go against the Microsoft Learn API to search for training modules.

Ok, so we created `functions` variables and a corresponding Python function, how do we tell the LLM how to map these two together so our Python function is called?

1. To see if we need to call a Python function, we need to look into the LLM response and see if `function_call` faz parte dela e chama a função apontada. Aqui está como você pode fazer a verificação mencionada abaixo:
...
- Criar tratamento de erros quando a chamada de função e/ou a chamada de API não retornar nenhum curso adequado.

Dica: Siga a página [documentação de referência da API do Learn](https://learn.microsoft.com/training/support/catalog-api-developer-reference?WT.mc_id=academic-105485-koreyst) para ver como e onde esses dados estão disponíveis.
